...
#study_model
#-------------------------------------------------------------------
CSV = 'Study_gaussian_new_new.csv'
#-------------------------------------------------------------------
...
#--------------------------------------------------------------
			...
			# print(q)
				...
				#CSV吐き出し
				list.extend([study_time[q],init_x[0],init_y[0],conc_pred[0][0]])
				#CSV吐き出し:time,x,y,conc
				with open(name2, 'a', newline='') as f:
					writer = csv.writer(f)	
					writer.writerow([list[0],list[1],list[2],list[3]])		
