...
#study_model
#-------------------------------------------------------------------
CSV = 'Train_data_set.csv'
#-------------------------------------------------------------------

csv_input = pd.read_csv(CSV,header=0)

#study_output:displacement of position (4step - 1step)
study_delta_head_x=csv_input.values[:-1, 2]
study_delta_head_y=csv_input.values[:-1, 3]
study_delta_tail_x=csv_input.values[:-1, 6]
study_delta_tail_y=csv_input.values[:-1, 9]

#study_input:current voltage
study_conc1=csv_input.values[:-1,13]
#study_input:voltage after 1 step
study_conc2=csv_input.values[:-1,14]
#study_input:voltage after 2 step
study_conc3=csv_input.values[:-1,15]
#study_input:voltage after 3 step
study_conc4=csv_input.values[:-1,16]
...
for inipos in range (-100,150,50):
	...
	totaltime = 0
	avetime = 0
	for ite in range(50): #iteration number
		#initial_Pos--------------------------------------------
